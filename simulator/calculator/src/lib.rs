...
pub fn parse_talisman(filename: &str, skill_name_dict: &HashMap<String, String>) -> Vec<Talisman> {
    let file = File::open(filename);
...
pub fn check_static_conditions<'a>(
    dm: &DataManager,
...
pub type BoxCalcEquipment<'a> = Box<dyn CalcEquipment<'a> + 'a>;

pub fn calculate_skillset<'a>(
    weapon_slots: Vec<i32>,
